...
export async function handleAIBuilderDetailedSubmit(ctx) {
  ...
  try {
    ...
    const overridden = applyDetailedOverride(cp, profile);
    overridden._detailed = profile; // round-trips into the form on re-edit
    ...
    await generateAndStore(env, refactorProject, overridden);

    await updateProject(env.DB, refactorProject.id, {
      company_profile_json: JSON.stringify(overridden),
